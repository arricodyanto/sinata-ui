...
function(n) {
    ...
    t.prototype._resetSidebarScroll = function() { n(".slimscroll-menu").slimscroll({ height: "auto", position: "right", size: "4px", color: "#9ea5ab", wheelStep: 5, touchScrollStep: 20 }) }, t.prototype.initMenu = function() {
        var e = this;
        ...
        if (n("#menu-bar").length)
            if ("topnav" !== t) new MetisMenu("#menu-bar"), e._resetSidebarScroll(), n("#menu-bar a").each(function() {
                ...
                var i = new MetisMenu("#menu-bar").on("shown.metisMenu", function(n) { window.addEventListener("click", function t(e) { n.target.contains(e.target) || (i.hide(n.detail.shownElement), window.removeEventListener("click", t)) }) });
    ...
    }, t.prototype.initLayout = function() { 768 <= this.$window.width() && this.$window.width() <= 1024 ? this.$body.addClass("left-side-menu-condensed") : 1 != this.$body.data("left-keep-condensed") && this.$body.removeClass("left-side-menu-condensed"), this.$body.hasClass("scrollable-layout") && n("#sidebar-menu").removeClass("slimscroll-menu") }, t.prototype.init = function() {
        var e = this;
        this.initLayout(), this.initMenu(), n.Components.init(), e.$window.on("resize", function(t) { t.preventDefault(), e.initLayout(), e._resetSidebarScroll() }), feather.replace()
